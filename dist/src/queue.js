...
export class MessageQueue {
    ...
    stmtEnqueueOutbound;
    stmtDequeueOutbound;
    stmtCheckOutboundDupe;
    stmtCheckSentDupe;
    stmtUpdateOutbound;
    stmtMarkOutboundProcessing;
    stmtRecordSent;
    stmtEnqueueInbound;
    stmtDequeueInbound;
    stmtUpdateInbound;
    stmtMarkInboundProcessing;
    stmtCheckInboundExists;
    constructor(dbPath) {
        this.dbPath = dbPath ?? path.join(process.env.HOME ?? '/root', '.openclaw', 'lark-queue.db');
     ...
     */
    enqueueOutbound(queueType, params) {
        const now = Date.now();
        const hash = this.hashContent(params.content);
        const dedupCutoff = now - DEDUP_WINDOW_MS;
        // Check for duplicate pending
        const existingPending = this.stmtCheckOutboundDupe.get(hash, params.chatId, dedupCutoff);
        if (existingPending) {
            return { enqueued: false, reason: 'duplicate_pending' };
        }
        // Check if already sent recently
        const existingSent = this.stmtCheckSentDupe.get(hash, params.chatId, dedupCutoff);
        if (existingSent) {
            return { enqueued: false, reason: 'already_sent' };
        }
        const result = this.stmtEnqueueOutbound.run(queueType, params.runId ?? '', params.sessionKey, params.chatId, params.content, hash, now, now, now);
     ...
     */
    dequeueOutbound(limit = 10) {
        return this.stmtDequeueOutbound.all(Date.now(), limit);
    }
    /**
     * Mark outbound message as being processed
     */
    markOutboundProcessing(id) {
        this.stmtMarkOutboundProcessing.run(Date.now(), id);
    }
    /**
     * Mark outbound message as completed
     */
    markOutboundCompleted(id, larkMessageId) {
        const now = Date.now();
        const msg = this.db.prepare('SELECT * FROM outbound_queue WHERE id = ?').get(id);
        this.stmtUpdateOutbound.run('completed', now, msg?.retries ?? 0, null, null, now, larkMessageId, id);
        if (msg) {
     ...
     */
    markOutboundRetry(id, errorMessage) {
        const now = Date.now();
        const msg = this.db.prepare('SELECT * FROM outbound_queue WHERE id = ?').get(id);
    ...
    enqueueInbound(params) {
        ...
        const existing = this.stmtCheckInboundExists.get(params.messageId);
        if (existing) {
     ...
     */
    dequeueInbound(limit = 5) {
        return this.stmtDequeueInbound.all(Date.now(), limit);
    }
    /**
     * Mark inbound message as being processed
     */
    markInboundProcessing(id) {
        this.stmtMarkInboundProcessing.run(Date.now(), id);
    }
    /**
     * Mark inbound message as completed (got response from Gateway)
     */
    markInboundCompleted(id, responseText) {
        const now = Date.now();
        const msg = this.db.prepare('SELECT * FROM inbound_queue WHERE id = ?').get(id);
        this.stmtUpdateInbound.run('completed', now, msg?.retries ?? 0, null, null, now, responseText, id);
     ...
     */
    markInboundRetry(id, errorMessage) {
        const now = Date.now();
        const msg = this.db.prepare('SELECT * FROM inbound_queue WHERE id = ?').get(id);
        const retries = (msg?.retries ?? 0) + 1;
        if (this.hasExceededMaxRetries(retries)) {
